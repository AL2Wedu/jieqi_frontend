extends Control
## 成就面板：列出成就与实时进度，可领取奖励。
## 数据来源 GET /v1/achievements → { items:[{achievement_id, name, description, category, reward, completed, claimed, progress}] }。
...
func _refresh() -> void:
	var res := await Backend.get_achievements()
...
func _add_row(item: Variant) -> void:
	...
	var completed: bool = bool(d.get("completed", false))
	var claimed: bool = bool(d.get("claimed", false))
	...
	fill.bg_color = Color(0.55, 0.8, 0.4, 1)
	...
	if completed and not claimed:
		...
		var a_id := str(d.get("achievement_id", ""))
		claim.pressed.connect(_claim.bind(a_id))
...
func _claim(achievement_id: String) -> void:
	_hint.visible = true
	_hint.text = "领取中…"
	var res := await Backend.claim_achievement(achievement_id)
	if not is_instance_valid(self):
		return
	if res.get("code", -1) != 0:
		_hint.visible = true
		_hint.text = str(res.get("message", "领取失败"))
		return
	_hint.text = ""
	assets_changed.emit()
	await _refresh()
